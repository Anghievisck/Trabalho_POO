...
* **Defeat Evaluation:** Monitors alien coordinates during rendering sweeps; if any active enemy intersects a player's ship or drops past the lower threshold (Y <= 50f), the gameplay loop terminates and redirects to `GameOverScreen`.
...
* **Flag-Based Entity Disposal:** Instantaneous memory deallocation during collision loops causes instability in the rendering thread. This project solves this by using boolean state flags (`isValid`, `isCollected`). Objects are marked as dead on impact and swept out of active lists during a separate cleanup pass driven by an `Iterator` loop.
...
Automated test procedures are built using the **JUnit** framework (found within `GameLogicTest`), allowing developers to validate physical equations and logic rules in a *headless* environment completely detached from LibGDX's graphical engine.

### Test Matrix Scope:
1. Verification of bounding box synchronization following rapid entity coordinate changes.
2. Collision evaluation accuracy between player bounds and downward-traveling projectiles.
3. Structural correctness of generated `Swarm` grids during level initialization.

**Results:** All automated unit tests pass successfully. The test verification confirms that collision logic ignores inactive entities (such as dead aliens or out-of-bounds bullets), avoiding false-positive collision loops.
...
1. Verify that your system has **Java Development Kit (JDK 26 or higher)** and the `make` utility installed.
